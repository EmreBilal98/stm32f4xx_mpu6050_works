...
#define MPU6050_INT_CONFIG_REG (uint8_t)55
#define MPU6050_INT_ENABLE_REG (uint8_t)56
#define MPU6050_INT_DISABLE_REG (uint8_t)57
#define MPU6050_INT_STATUS_REG (uint8_t)58
...
MPU6050_status mpu6050_interrupt_config(I2C_HandleTypeDef *hi2c,mpu6050_interrupt_config_t level){


	uint8_t response = 0;

	if(mpu6050_read_byte(hi2c,MPU6050_INT_CONFIG_REG,&response) != MPU6050_OK)
		return MPU6050_ERR;

	//clear level bit
	response &= ~(0x80);
	response |= (uint8_t)level;

	if(mpu6050_write_byte(hi2c, MPU6050_INT_CONFIG_REG,(uint8_t)response)!= MPU6050_OK){
		return MPU6050_ERR;
	}
...
MPU6050_status mpu6050_enable_interrupts(I2C_HandleTypeDef *hi2c,mpu6050_interrupt_t interrupt){

	uint8_t response = 0;

	if(mpu6050_read_byte(hi2c,MPU6050_INT_ENABLE_REG,&response) != MPU6050_OK)
		return MPU6050_ERR;

	response |= (uint8_t)interrupt;

	if(mpu6050_write_byte(hi2c, MPU6050_INT_ENABLE_REG,(uint8_t)response)!= MPU6050_OK){
		return MPU6050_ERR;
	}
...
MPU6050_status mpu6050_disable_interrupts(I2C_HandleTypeDef *hi2c,mpu6050_interrupt_t interrupt){

	uint8_t response = 0;

	if(mpu6050_read_byte(hi2c,MPU6050_INT_DISABLE_REG,&response) != MPU6050_OK)
		return MPU6050_ERR;

	response |= (uint8_t)interrupt;

	if(mpu6050_write_byte(hi2c, MPU6050_INT_DISABLE_REG,(uint8_t)interrupt)!= MPU6050_OK){
		return MPU6050_ERR;
	}
...
static MPU6050_status get_interrupt_status(I2C_HandleTypeDef *hi2c,uint8_t *data){
	//read the interrupt status register MPU6050_INT_CONFIG_REG

	if(mpu6050_read_byte(hi2c,MPU6050_INT_STATUS_REG,data) != MPU6050_OK)
		return MPU6050_ERR;

	return MPU6050_OK;
}

static MPU6050_status get_interrupt_settings(I2C_HandleTypeDef *hi2c,uint8_t *data){
	//read the interrupt status register MPU6050_INT_ENABLE_REG

	if(mpu6050_read_byte(hi2c,MPU6050_INT_ENABLE_REG,data) != MPU6050_OK)
		return MPU6050_ERR;
